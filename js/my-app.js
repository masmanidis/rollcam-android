			...
			if( glevel ) {
				...
				function() {
					...
					bluetoothSerial.isConnected(
						...
						function() {
							// i infrared   int steps, int timeout, int shoots, String camtype [S], String ircode, String frequency  (i,3,1,S,0xb4b8f,20)
							// v video      int steps, int timeout, int inSpeed  (v,2,1,33)   (speed 33,22,16)
							// m manual     int steps  (m)
							// z flash      int steps, int timeout  (z,1,2)
							// n normal     int steps, int timeout (n,1,2
							// f-front b-back e-exit s-stop r-restart w-reset  (direct commands)
							$$("#stepDisplay").html(0);
							$$("#totalSteps").html($$('select[name="rollCamSteps"] option:checked')[0].value);
							var afterFlash = $$('#moveAfterFlash').prop('checked') ? 'z':'';
							var useInfrared = $$('#useInfrared').prop('checked') ? 'i':'';
							var useVideo = $$('#useVideo').prop('checked') ? 'v':'';
							var operator = '';
							var extraArguments = '';
							if( afterFlash == 'z' ) {
								operator = 'z';
								
							} else if( useInfrared == 'i' )	 {
								operator = 'i';
								extraArguments = ','+$$('select[name="InfraredModels"] option:checked')[0].value;
								localStorage.setItem('model',$$('select[name="InfraredModels"] option:checked')[0].value);	
							} else if(useVideo == 'v') {
								
								operator = 'v';
								extraArguments = ','+$$('select[name="secPerRevol"] option:checked')[0].value;
								localStorage.setItem('speed',$$('select[name="secPerRevol"] option:checked')[0].value);	
							} else if( afterFlash == '' && useInfrared == '' && useVideo == '' ) {
								operator = 'n';
							 	if( $$('select[name="rollCamDelay"] option:checked')[0].value == '0' ) { 
									operator = 'm';
								}
							}
							if( operator != '' )  {
								bluetoothSerial.write(	operator+","+$$('select[name="rollCamSteps"] option:checked')[0].value+","+
														$$('select[name="rollCamDelay"] option:checked')[0].value+extraArguments+"\n");
								$$('#tab2link').click();
							} else {
								myApp.alert("Settting are not send successfully");		
								return;
							}
							if( useInfrared == 'i' ) {
								$$("#timerClock").show();
			...
			if( localStorage.getItem('infrared') ) {
				if( localStorage.getItem('infrared') === 'true' ) {
						...
						if(localStorage.getItem('model')){ 
							$$("#InfraredModels").val( localStorage.getItem('model') ).change();
						} else {
							$$("#InfraredModels").val( modelPos ).change();
						}
			...
			if( localStorage.getItem('video') ) {
				if( localStorage.getItem('video') === 'true' ) {
						...
						if(localStorage.getItem('speed')){ 
							$$("#secPerRevol").val( localStorage.getItem('speed') ).change();
						} else {
							$$("#secPerRevol").val( speedPos ).change();
						}
...
/* var rollCamSteps = myApp.picker({
	...
	$$("#secPerRevol").on('change', function(){
